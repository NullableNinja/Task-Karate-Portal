<script lang="ts">
  ...
  type AttendanceRecord = { attendanceId: number; studentId: number; studentName: string; checkedInAtUtc: string; notes?: string };
  ...
  async function checkIn() { recording = true; error = ''; notice = ''; try { await api('/api/portal-admin/attendance', { method: 'POST', body: JSON.stringify({ studentId: Number(studentId), sessionId: Number(classSessionId) }) }); notice = 'Attendance recorded in the portal database.'; studentId = ''; studentSearch = ''; await selectSession(); } catch (e) { error = e instanceof Error ? e.message : 'Could not record attendance.'; } finally { recording = false; } }
  onMount(load);
</script>
...
<section class="card attendance-workflow"><div class="workflow-step"><span class="workflow-number">1</span><div><h3>Choose the class date</h3><p class="muted">Only sessions on this date appear in the next step.</p><label>Date<input type="date" bind:value={selectedDate} on:change={load} /></label></div></div><div class="workflow-step"><span class="workflow-number">2</span><div class="workflow-grow"><h3>Choose the session</h3><p class="muted">This is the real class occurrence students are attending.</p><select bind:value={classSessionId} on:change={selectSession} disabled={loading || !sessions.length}><option value="">{loading ? 'Loading sessions…' : sessions.length ? 'Select a class session' : 'No sessions on this date'}</option>{#each sessions as session}<option value={session.sessionId}>{session.className} · {session.startTime ?? 'Time not set'}{session.location ? ` · ${session.location}` : ''}{session.cancelled ? ' · Cancelled' : ''}</option>{/each}</select></div></div><div class="workflow-step"><span class="workflow-number">3</span><div class="workflow-grow"><h3>Check in the student</h3><p class="muted">Search narrows the roster; the second field is the student you will record.</p><div class="attendance-student-controls"><input bind:value={studentSearch} placeholder="Find a student by name…" aria-label="Find a student by name" disabled={!classSessionId} /><select bind:value={studentId} disabled={!classSessionId || !visibleStudents.length} aria-label="Student to check in"><option value="">{visibleStudents.length ? 'Select student to check in' : 'No matching active students'}</option>{#each visibleStudents as student}<option value={student.studentId}>{student.displayName} · {student.rankName ?? 'Rank not assigned'}</option>{/each}</select></div>{#if !students.length && !loading}<p class="empty-help">No active portal students are available. <a href="/staff/students">Add or activate a student first.</a></p>{:else if !visibleStudents.length && studentSearch}<p class="empty-help">No active students match “{studentSearch}”. Clear the search to see the roster.</p>{/if}</div></div><div class="workflow-actions"><button class="button attendance-submit" disabled={!classSessionId || !studentId || recording || selectedSession?.cancelled}>{recording ? 'Recording…' : 'Record attendance'}</button>{#if selectedSession?.cancelled}<span class="muted">Cancelled sessions cannot accept check-ins.</span>{/if}</div></section>
