<script lang="ts">
  ...
  $: currentRankName = profile?.rankName ?? 'White Belt';
  $: nextRankName = nextRank(currentRankName);
  ...
  <StudentShell {session} active="training">
    ...
    {#if loading}<div class="student-panel loading-panel">Loading your requirements…</div>{:else}
      ...
      {#if activeTrack === 'karate'}
        <section class="student-panel training-rank-summary"><div><span class="student-eyebrow">CURRENT RANK</span><strong>{currentRankName}</strong></div><span class="rank-transition" aria-hidden="true">→</span><div><span class="student-eyebrow">NEXT TEST</span><strong>{nextRankName === currentRankName ? 'Degree development' : nextRankName}</strong></div><a class="text-link" href="/student/profile">View profile →</a></section>
        <div class="student-grid two-column training-top-grid"><section class="student-panel training-hero"><span class="belt-graphic" style={`--belt-color:${karatePlan.color}`}><span class="belt-knot"></span><span class="belt-strip"></span></span><span class="student-eyebrow">NEXT BELT TO EARN · {trackLabel}</span><h2>{nextRankName}</h2><p>{karatePlan.promotionGuide}</p></section><section class="student-panel stripe-panel"><div class="panel-title"><span>STRIPE PROGRESS</span><a href="/student/profile">View profile →</a></div><div class="stripe-track" aria-label={`${profile.classesIntoStripe} of ${profile.classesPerStripe || 0} classes toward next stripe`}><span style={`width:${stripePercent}%`}></span></div><div class="stripe-summary"><strong>{profile.classesToNextStripe || 'Instructor tracked'} {profile.classesToNextStripe ? (profile.classesToNextStripe === 1 ? 'class' : 'classes') : ''} until next stripe</strong><span>{profile.classesPerStripe ? `${profile.classesIntoStripe} of ${profile.classesPerStripe} completed` : 'Black belt degrees are tracked by your instructor.'}</span></div></section></div>
      {:else}
