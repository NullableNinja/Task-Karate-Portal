(async () => {
  ...
  const serverMode = await window.TaskKarateStore?.serverMode?.() || false;
  let roster = await window.TaskKarateStore.getRoster();
  ...
  document.querySelector("[data-roster-source]").textContent = serverMode ? "SQLite desk server" : "IndexedDB browser fallback";
  document.querySelector("#roster-add-form").addEventListener("submit", async (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); const student = await window.TaskKarateStore.addStudent({ ...values, id: values.id.toLowerCase().replace(/[^a-z0-9-]+/g, "-") }); roster = [...roster, student].sort((a, b) => (a.name || "").localeCompare(b.name || "")); renderRoster(); show(document.querySelector("[data-roster-add-status]"), "Student added to the local roster."); event.currentTarget.reset(); });
  ...
  function renderRoster() { document.querySelector("[data-admin-roster]").innerHTML = roster.map((student) => `<div class="admin-roster-row" data-roster-row="${esc(student.id)}"><div class="admin-roster-row__identity"><strong>${esc(student.displayName || student.name)}</strong><small>${esc(student.rank)} · ${esc(student.ageGroup || "teensAdults")}</small></div><div class="admin-roster-row__fields"><input aria-label="Belt size for ${esc(student.name)}" data-roster-field="beltSize" value="${esc(student.beltSize || "")}" placeholder="Belt size"><input aria-label="Uniform size for ${esc(student.name)}" data-roster-field="uniformSize" value="${esc(student.uniformSize || "")}" placeholder="Uniform size"><select aria-label="Age group for ${esc(student.name)}" data-roster-field="ageGroup"><option value="kids" ${student.ageGroup === "kids" ? "selected" : ""}>Kids</option><option value="teensAdults" ${student.ageGroup === "teensAdults" ? "selected" : ""}>Teens &amp; Adults</option><option value="staff" ${student.ageGroup === "staff" ? "selected" : ""}>Staff</option></select></div><div class="admin-roster-row__actions"><button class="admin-button admin-button--small admin-button--blue" type="button" data-save-roster="${esc(student.id)}">Save</button><button class="admin-button admin-button--small admin-button--outline" type="button" data-deactivate-roster="${esc(student.id)}">Deactivate</button></div></div>`).join(""); document.querySelectorAll("[data-save-roster]").forEach((button) => button.addEventListener("click", async () => { const row = button.closest("[data-roster-row]"); const changes = Object.fromEntries([...row.querySelectorAll("[data-roster-field]")].map((field) => [field.dataset.rosterField, field.value])); await window.TaskKarateStore.saveRosterOverride(button.dataset.saveRoster, changes); const target = roster.find((student) => student.id === button.dataset.saveRoster); Object.assign(target, changes); const previous = button.textContent; button.textContent = "Saved"; window.setTimeout(() => { button.textContent = previous; }, 1600); })); document.querySelectorAll("[data-deactivate-roster]").forEach((button) => button.addEventListener("click", async () => { await window.TaskKarateStore.deactivateStudent(button.dataset.deactivateRoster); roster = roster.filter((student) => student.id !== button.dataset.deactivateRoster); renderRoster(); })); }
})();
