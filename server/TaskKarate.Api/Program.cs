...
{
    if (!request.DisclaimerAccepted) return Results.Json(new { title = "Staff acknowledgment required", detail = "Review and accept the local staff-use acknowledgment before opening student records." }, statusCode: StatusCodes.Status428PreconditionRequired);
    var identifier = (request.Username ?? request.Email)?.Trim();
    if (string.IsNullOrWhiteSpace(identifier) || string.IsNullOrWhiteSpace(request.Password)) return Results.ValidationProblem(new Dictionary<string, string[]> { ["credentials"] = ["Enter your staff username or email and password."] });
...
staff.MapPost("/class-templates", async (TemplateRequest request, ApplicationDbContext db, HttpContext context, AuditService audit) => { if (request.ProgramId == Guid.Empty || string.IsNullOrWhiteSpace(request.Name) || string.IsNullOrWhiteSpace(request.DayOfWeek) || request.DurationMinutes is < 1 or > 480) return Results.ValidationProblem(new Dictionary<string, string[]> { ["template"] = ["Program, name, day, and duration are required."] }); if (!new[] { "Class", "Seminar", "Private lesson" }.Contains(request.ClassType)) return Results.ValidationProblem(new Dictionary<string, string[]> { ["classType"] = ["Choose Class, Seminar, or Private lesson."] }); if (!await db.Programs.AnyAsync(x => x.Id == request.ProgramId && x.IsActive)) return Results.ValidationProblem(new Dictionary<string, string[]> { ["programId"] = ["Program does not exist."] }); var template = new ClassTemplate { ProgramAreaId = request.ProgramId, Name = request.Name.Trim(), DayOfWeek = request.DayOfWeek.Trim(), StartTime = request.StartTime.Trim(), DurationMinutes = request.DurationMinutes, BeltScope = request.BeltScope?.Trim(), ClassType = request.ClassType, AppointmentOnly = request.AppointmentOnly }; db.ClassTemplates.Add(template); await db.SaveChangesAsync(); await audit.RecordAsync(context, "Create", "ClassTemplate", template.Id, new { request.ClassType }); return Results.Created($"/api/class-templates/{template.Id}", new { template.Id }); });
staff.MapGet("/enrollments", async (ApplicationDbContext db, Guid? classTemplateId) => Results.Ok(await db.Enrollments.AsNoTracking().Include(x => x.Student).Include(x => x.ClassTemplate).Where(x => x.IsActive && (classTemplateId == null || x.ClassTemplateId == classTemplateId)).OrderBy(x => x.ClassTemplate.Name).ThenBy(x => x.Student.LastName).Select(x => new EnrollmentDto(x.Id, x.StudentId, x.Student.FirstName + " " + x.Student.LastName, x.ClassTemplateId, x.ClassTemplate.Name, x.IsActive)).ToListAsync()));
staff.MapPost("/enrollments", async (EnrollmentRequest request, ApplicationDbContext db, HttpContext context, AuditService audit) =>
{
    if (request.StudentId == Guid.Empty || request.ClassTemplateId == Guid.Empty) return Results.ValidationProblem(new Dictionary<string, string[]> { ["enrollment"] = ["Choose an active student and class template."] });
    if (!await db.Students.AnyAsync(x => x.Id == request.StudentId && x.IsActive) || !await db.ClassTemplates.AnyAsync(x => x.Id == request.ClassTemplateId && x.IsActive)) return Results.ValidationProblem(new Dictionary<string, string[]> { ["enrollment"] = ["The student or class template does not exist or is inactive."] });
    var enrollment = await db.Enrollments.FirstOrDefaultAsync(x => x.StudentId == request.StudentId && x.ClassTemplateId == request.ClassTemplateId);
    if (enrollment is null) { enrollment = new Enrollment { StudentId = request.StudentId, ClassTemplateId = request.ClassTemplateId }; db.Enrollments.Add(enrollment); }
    else if (enrollment.IsActive) return Results.Conflict(new { title = "Student already enrolled" });
    else { enrollment.IsActive = true; enrollment.UpdatedAtUtc = DateTime.UtcNow; }
    await db.SaveChangesAsync(); await audit.RecordAsync(context, "Create", "Enrollment", enrollment.Id, new { request.StudentId, request.ClassTemplateId }); return Results.Created($"/api/enrollments/{enrollment.Id}", new { enrollment.Id });
});
staff.MapDelete("/enrollments/{id:guid}", async (Guid id, ApplicationDbContext db, HttpContext context, AuditService audit) => { var enrollment = await db.Enrollments.FindAsync(id); if (enrollment is null || !enrollment.IsActive) return Results.NotFound(); enrollment.IsActive = false; enrollment.UpdatedAtUtc = DateTime.UtcNow; await db.SaveChangesAsync(); await audit.RecordAsync(context, "Deactivate", "Enrollment", id, new { enrollment.StudentId, enrollment.ClassTemplateId }); return Results.NoContent(); });
staff.MapGet("/sessions", async (ApplicationDbContext db, DateTime? date) => { var day = (date ?? DateTime.UtcNow).Date; return Results.Ok(await db.ClassSessions.AsNoTracking().Include(x => x.ClassTemplate).Where(x => x.SessionDateUtc >= day && x.SessionDateUtc < day.AddDays(1)).OrderBy(x => x.ClassTemplate.StartTime).Select(x => new SessionDto(x.Id, x.ClassTemplateId, x.ClassTemplate.Name, x.ClassTemplate.StartTime, x.ClassTemplate.DurationMinutes, x.SessionDateUtc, x.IsCancelled, string.IsNullOrWhiteSpace(x.ClassTemplate.ClassType) ? "Class" : x.ClassTemplate.ClassType, x.AssignedStudentId)).ToListAsync()); });
staff.MapPost("/sessions", async (SessionRequest request, ApplicationDbContext db, HttpContext context, AuditService audit) => { var template = await db.ClassTemplates.FirstOrDefaultAsync(x => x.Id == request.ClassTemplateId && x.IsActive); if (template is null) return Results.ValidationProblem(new Dictionary<string, string[]> { ["classTemplateId"] = ["Class template does not exist."] }); if (template.ClassType == "Private lesson" && request.AssignedStudentId is null) return Results.ValidationProblem(new Dictionary<string, string[]> { ["assignedStudentId"] = ["A private lesson must be assigned to an active student."] }); if (request.AssignedStudentId is not null && !await db.Students.AnyAsync(x => x.Id == request.AssignedStudentId && x.IsActive)) return Results.ValidationProblem(new Dictionary<string, string[]> { ["assignedStudentId"] = ["Assigned student does not exist or is inactive."] }); var date = request.SessionDateUtc.Date; if (await db.ClassSessions.AnyAsync(x => x.ClassTemplateId == template.Id && x.SessionDateUtc == date)) return Results.Conflict(new { title = "Class session already exists" }); var session = new ClassSession { ClassTemplateId = template.Id, SessionDateUtc = DateTime.SpecifyKind(date, DateTimeKind.Utc), Notes = request.Notes?.Trim(), AssignedStudentId = request.AssignedStudentId }; db.ClassSessions.Add(session); await db.SaveChangesAsync(); await audit.RecordAsync(context, "Create", "ClassSession", session.Id, new { request.AssignedStudentId, template.ClassType }); return Results.Created($"/api/sessions/{session.Id}", new { session.Id }); });
...
public sealed record LoginRequest(string? Email, string Password, bool RememberMe = false, bool DisclaimerAccepted = false, string? Username = null);
...
public sealed record TemplateRequest(Guid ProgramId, string Name, string DayOfWeek, string StartTime, int DurationMinutes, string? BeltScope, string ClassType = "Class", bool AppointmentOnly = false);
public sealed record SessionRequest(Guid ClassTemplateId, DateTime SessionDateUtc, string? Notes, Guid? AssignedStudentId = null);
public sealed record EnrollmentRequest(Guid StudentId, Guid ClassTemplateId);
...
public sealed record TemplateDto(Guid Id, string Name, string DayOfWeek, string StartTime, int DurationMinutes, string? BeltScope, string ClassType, bool AppointmentOnly);
public sealed record SessionDto(Guid Id, Guid ClassTemplateId, string Name, string StartTime, int DurationMinutes, DateTime SessionDateUtc, bool IsCancelled, string? ClassType, Guid? AssignedStudentId);
public sealed record EnrollmentDto(Guid Id, Guid StudentId, string StudentName, Guid ClassTemplateId, string ClassName, bool IsActive);
