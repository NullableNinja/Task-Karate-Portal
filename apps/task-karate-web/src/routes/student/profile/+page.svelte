<script lang="ts">
  ...
  async function changePassword() { changingPassword = true; passwordError = ''; passwordNotice = ''; try { await api('/api/student/auth/password', { method: 'POST', body: JSON.stringify(passwordForm) }); passwordForm = { currentPassword: '', newPassword: '', confirmPassword: '' }; passwordNotice = 'Password changed. Use the new password the next time you sign in.'; } catch (e) { passwordError = apiError(e); } finally { changingPassword = false; } }
  async function changePin() { changingPin = true; pinError = ''; pinNotice = ''; try { await api('/api/student/auth/pin', { method: 'POST', body: JSON.stringify(pinForm) }); pinForm = { currentPin: '', newPin: '', confirmPin: '' }; pinNotice = 'Check-in PIN changed.'; } catch (e) { pinError = apiError(e); } finally { changingPin = false; } }
</script>
...
{#if session}
  <StudentShell {session} active="profile">
    ...
    {#if error}<div class="error" role="alert">{error}</div>{:else if !profile}<div class="student-panel loading-panel">Loading your profile…</div>{:else}
      ...
      <section class="student-panel password-panel"><div class="panel-title"><span>ACCOUNT PASSWORD</span><span class="muted">For hub access</span></div><p class="muted">Use your password to sign into the student hub from the internet. Your current password is required.</p>{#if passwordError}<div class="error" role="alert">{passwordError}</div>{/if}{#if passwordNotice}<div class="status" role="status">{passwordNotice}</div>{/if}<form class="profile-form password-form" on:submit|preventDefault={changePassword}><label>Current password<input type="password" bind:value={passwordForm.currentPassword} autocomplete="current-password" required /></label><label>New password<input type="password" bind:value={passwordForm.newPassword} minlength="12" autocomplete="new-password" required /><small class="muted">Use at least 12 characters with uppercase, lowercase, and a number.</small></label><label>Confirm new password<input type="password" bind:value={passwordForm.confirmPassword} minlength="12" autocomplete="new-password" required /></label><div class="profile-form-actions"><button class="primary-button" type="submit" disabled={changingPassword}>{changingPassword ? 'Changing…' : 'Change password'}</button></div></form></section>
      <section class="student-panel password-panel"><div class="panel-title"><span>SCHEDULE CHECK-IN PIN</span><span class="muted">For supervised attendance</span></div><p class="muted">Use this separate 4–6 digit PIN when checking in through the dojo schedule. Your hub password is never used at the front desk.</p>{#if pinError}<div class="error" role="alert">{pinError}</div>{/if}{#if pinNotice}<div class="status" role="status">{pinNotice}</div>{/if}<form class="profile-form password-form" on:submit|preventDefault={changePin}><label>Current PIN<input type="password" bind:value={pinForm.currentPin} autocomplete="current-password" required /></label><label>New PIN<input type="password" bind:value={pinForm.newPin} minlength="4" maxlength="6" pattern="[0-9][0-9][0-9][0-9][0-9]?[0-9]?" autocomplete="new-password" required /><small class="muted">Use 4–6 digits.</small></label><label>Confirm new PIN<input type="password" bind:value={pinForm.confirmPin} minlength="4" maxlength="6" pattern="[0-9][0-9][0-9][0-9][0-9]?[0-9]?" autocomplete="new-password" required /></label><div class="profile-form-actions"><button class="primary-button" type="submit" disabled={changingPin}>{changingPin ? 'Changing…' : 'Change check-in PIN'}</button></div></form></section>
    {/if}
  </StudentShell>
{/if}
