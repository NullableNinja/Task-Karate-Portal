<script lang="ts">
  ...
  $: visibleFeed = feed.filter((item) => { const matchesQuery = !feedQuery.trim() || `${item.authorName} ${item.text}`.toLowerCase().includes(feedQuery.trim().toLowerCase()); const matchesFilter = feedFilter === 'all' || (feedFilter === 'school' ? item.postType === 'news' : feedFilter === 'connections' ? item.postType !== 'news' : bookmarks.includes(item.postId)); return matchesQuery && matchesFilter; });
...
{#if session}
  <StudentShell {session} active="social">
    ...
    <div class="social-feed-layout">
        ...
        <section class="student-panel feed-controls"><div class="feed-filter-row" aria-label="Feed filters">{#each [{id:'all',label:'All updates'},{id:'connections',label:'Connections'},{id:'school',label:'School news'},{id:'saved',label:'Saved'}] as filter}<button type="button" class:active={feedFilter === filter.id} on:click={() => feedFilter = filter.id}>{filter.label}</button>{/each}</div><input bind:value={feedQuery} placeholder="Search the dojo feed…" aria-label="Search the dojo feed" /></section>
