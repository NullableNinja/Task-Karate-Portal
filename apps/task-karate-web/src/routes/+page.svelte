<script lang="ts">
  ...
  onMount(async () => { try { [schedule, announcements] = await Promise.all([api<any[]>('/api/public/schedule'), api<Content[]>('/api/public/announcements')]); } catch (e) { error = e instanceof Error ? e.message : 'The public API is unavailable.'; } finally { loading = false; } });
</script>
...
<main class="public-shell">
  ...
  {#if loading}<p class="status">Loading current information…</p>{:else if error}<div class="error"><strong>Live data unavailable.</strong><p>{error}</p></div>{:else}
    <section class="card"><div class="section-heading"><div><span class="eyebrow">UPCOMING</span><h2>Class schedule</h2></div></div>{#if schedule.length === 0}<p class="muted">No published sessions are available yet.</p>{:else}<div class="schedule-grid">{#each schedule as item}<article class="schedule-item"><strong>{new Date(item.date).toLocaleDateString()}</strong><span>{item.startTime} · {item.durationMinutes} min</span><b>{item.className}</b><small>{item.program}</small></article>{/each}</div>{/if}</section>
