...
The student service owns the portal-facing operational tables in the runtime database: `students`, `guardians`, `student_guardians`, program memberships, rank/profile/achievement data, classes, sessions, attendance, social activity, and the portal-specific `portal_programs`, `portal_class_templates`, `portal_enrollments`, and `portal_content` tables. The active staff and student workflows read and write this model. Development normalization repairs duplicate guardian seed rows and prevents an IS3 membership from being shown for a non-Teen/Adult student.

The physical database consolidation and active-workflow cutover are complete. The `platform_*` namespace is an intentional compatibility boundary for staff Identity and older EF contracts; it is no longer the source of truth for the operational portal screens. The next production-hardening phase is to retire unused compatibility entities and remove the one-time importer after a verified production backup and cutover review.
