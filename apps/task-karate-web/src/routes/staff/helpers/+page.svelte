...
<div class="toolbar"><div><span class="eyebrow">HELPER ROSTER</span><h2>Volunteer coverage</h2><p class="muted">Sign yourself up before class, then see staff volunteers and student helpers together for every upcoming session.</p></div><div class="helper-summary"><strong>{volunteerCount}</strong><span>staff volunteer signups</span></div></div>
...
{:else}
  <div class="helper-days">
    {#each days as day}
      <section class="helper-day-group">
        <div class="helper-day-heading"><div><span class="eyebrow">{isToday(day.date) ? 'TODAY' : 'UPCOMING'}</span><h3>{dateLabel(day.date)}</h3></div><span class="pill">{day.sessions.length} session{day.sessions.length === 1 ? '' : 's'}</span></div>
        <div class="helper-session-list">
          {#each day.sessions as session}
            <article class="card helper-session-card">
