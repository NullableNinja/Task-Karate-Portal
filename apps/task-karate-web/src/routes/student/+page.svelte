<script lang="ts">
  ...
  let session: StudentSession | null = null; let profile: any = null; let error = ''; let loading = true;
  onMount(async () => { session = await requireStudent(); if (!session) return; try { profile = await api('/api/student/profile'); } catch (e) { error = apiError(e); } finally { loading = false; } });
...
{#if session}
  <StudentShell {session} active="status">
    ...
    {#if loading}<div class="student-panel">Loading your training record…</div>{:else if error}<div class="student-panel error">{error}</div>{:else}
      ...
      <section class="student-panel"><div class="panel-title"><span>RECENT ATTENDANCE</span><a href="/schedule">Check in →</a></div><div class="attendance-dots">{#each profile?.attendanceDates ?? [] as date}<span title={date} class="present-dot"></span>{/each}{#if (profile?.attendanceDates ?? []).length === 0}<p class="muted">Your class check-ins will appear here.</p>{/if}</div></section>
    {/if}
  </StudentShell>
{/if}
