<script lang="ts">
  ...
  onMount(async () => { session = await requireStudent(); if (!session) return; try { await Promise.all([loadFriends(), loadFeed(), loadBookmarks()]); if (new URL(window.location.href).searchParams.get('focus') === 'messages') { const firstFriend = friends.find((friend) => friend.status === 'accepted'); if (firstFriend) await selectFriend(firstFriend); } } catch (e) { error = apiError(e); } finally { loading = false; } });
  async function search() { searchMessage = ''; if (query.trim().length < 2) { results = []; searchMessage = 'Enter at least two characters to search the roster.'; return; } try { results = await api<any[]>(`/api/student/social/search?q=${encodeURIComponent(query)}`); if (!results.length) searchMessage = 'No matching student profiles found.'; } catch (e) { error = apiError(e); } }
  ...
  async function submitComment(postId: number) { const textValue = commentDrafts[postId]?.trim(); if (!textValue) return; try { await api(`/api/student/feed/${postId}/comments`, { method: 'POST', body: JSON.stringify({ text: textValue }) }); commentDrafts = { ...commentDrafts, [postId]: '' }; comments = { ...comments, [postId]: await api<any[]>(`/api/student/feed/${postId}/comments`) }; feed = feed.map((item) => item.postId === postId ? { ...item, commentCount: (item.commentCount ?? 0) + 1 } : item); expandedComments = { ...expandedComments, [postId]: true }; } catch (e) { error = apiError(e); } }
  async function toggleBookmark(postId: number) { try { await api(`/api/student/feed/${postId}/bookmark`, { method: 'POST' }); await loadBookmarks(); } catch (e) { error = apiError(e); } }
  $: visibleFeed = feed.filter((item) => { const matchesQuery = !feedQuery.trim() || `${item.authorName} ${item.text}`.toLowerCase().includes(feedQuery.trim().toLowerCase()); const matchesFilter = feedFilter === 'all' || (feedFilter === 'connections' ? item.postType === 'student' : bookmarks.includes(item.postId)); return matchesQuery && matchesFilter; });
...
{#if session}
  <StudentShell {session} active="social">
    ...
    <div class="social-feed-layout">
        ...
        <section class="student-panel post-composer"><div class="panel-title"><span>SHARE WITH THE DOJO</span><span class="muted">{postText.length}/2000 · text posts only</span></div><form on:submit|preventDefault={publish}><textarea bind:value={postText} maxlength="2000" placeholder="What are you working on this week?" aria-label="Create a dojo post"></textarea><div class="composer-footer"><span class="muted">Keep it respectful and training-focused.</span><button class="primary-button" disabled={publishing || !postText.trim()}>{publishing ? 'Posting…' : 'Post update'}</button></div></form></section>
        <section class="student-panel feed-controls"><div class="feed-filter-row" aria-label="Feed filters">{#each [{id:'all',label:'All updates'},{id:'connections',label:'Connections'},{id:'saved',label:'Saved'}] as filter}<button type="button" class:active={feedFilter === filter.id} on:click={() => feedFilter = filter.id}>{filter.label}</button>{/each}</div><input bind:value={feedQuery} placeholder="Search the dojo feed…" aria-label="Search the dojo feed" /></section>
        <section class="feed-stream" aria-label="Dojo feed">
          {#if loading}<div class="student-panel loading-panel">Loading the dojo feed…</div>{:else if visibleFeed.length === 0}<div class="student-panel empty-panel"><span class="rank-orb">✦</span><h2>{feedFilter === 'saved' ? 'Nothing saved yet.' : 'No updates match that view.'}</h2><p>Try another filter, search term, or share a training note with the dojo.</p></div>{:else}{#each visibleFeed as item}<article class="student-panel feed-card"><div class="feed-author"><span class="mini-avatar">{item.authorName?.slice(0, 2).toUpperCase()}</span><div><strong>{item.authorName}</strong><small>{item.postType === 'news' ? 'Task Karate update' : (item.rankName ?? 'Student')} · {when(item.createdAt)}</small></div><button class="bookmark-button" class:saved={bookmarks.includes(item.postId)} type="button" title={bookmarks.includes(item.postId) ? 'Remove saved post' : 'Save post'} aria-label={bookmarks.includes(item.postId) ? 'Remove saved post' : 'Save post'} on:click={() => toggleBookmark(item.postId)}>{bookmarks.includes(item.postId) ? '★' : '☆'}</button></div><p>{item.text}</p><div class="feed-actions"><div class="reaction-row" aria-label={`Reactions for ${item.authorName}'s update`}>{#each reactionOptions as option}{@const existing = item.reactions?.find((reaction: any) => reaction.code === option.code)}<button class:selected={existing?.selected} class="reaction-button" type="button" title={option.label} aria-label={`${option.label}${existing?.selected ? ', selected' : ''}`} on:click={() => react(item.postId, option.code)}><span class="reaction-icon">{option.icon}</span><span>{option.label}</span><b>{existing?.count ?? 0}</b></button>{/each}</div><button class="comment-toggle" type="button" on:click={() => toggleComments(item.postId)}>{expandedComments[item.postId] ? 'Hide comments' : `${item.commentCount ?? 0} ${item.commentCount === 1 ? 'comment' : 'comments'}`}</button></div>{#if expandedComments[item.postId]}<div class="comment-area"><div class="comment-list">{#each comments[item.postId] ?? [] as comment}<div class="comment-item"><span class="mini-avatar">{comment.authorName?.slice(0, 2).toUpperCase()}</span><div><strong>{comment.authorName}</strong><p>{comment.text}</p><small>{when(comment.createdAt)}</small></div></div>{:else}<p class="muted">No comments yet. Be the first to encourage this training update.</p>{/each}</div><form class="comment-compose" on:submit|preventDefault={() => submitComment(item.postId)}><input bind:value={commentDrafts[item.postId]} maxlength="1000" placeholder="Add an encouraging comment…" aria-label={`Comment on ${item.authorName}'s update`} /><button class="small-action" type="submit">Send</button></form></div>{/if}</article>{/each}{/if}
        </section>
      </section>
      <aside class="social-sidebar">
        <section class="student-panel friends-panel"><div class="panel-title"><span>FRIENDS & REQUESTS</span></div><form class="search-row" on:submit|preventDefault={search}><input bind:value={query} placeholder="Find a student…" aria-label="Find a student" /><button class="outline-button">Search</button></form>{#if searchMessage}<p class="muted search-message" role="status">{searchMessage}</p>{/if}{#if results.length}<div class="search-results">{#each results as item}<div class="person-row"><span class="mini-avatar">{item.displayName?.slice(0,2).toUpperCase()}</span><span><strong>{item.displayName}</strong><small>{item.rankName ?? 'Student'}</small></span><button class="small-action" on:click={() => request(item.studentId)}>Add friend</button></div>{/each}</div>{/if}<div class="friend-list">{#if loading}<p class="muted">Loading…</p>{:else if friends.length === 0}<p class="muted">No connections yet. Search the dojo to send a request.</p>{:else}{#each friends as friend}<div class="person-row" class:selected={selected?.studentId === friend.studentId}><button class="person-select" on:click={() => selectFriend(friend)}><span class="mini-avatar">{friend.displayName?.slice(0,2).toUpperCase()}</span><span><strong>{friend.displayName}</strong><small>{friend.rankName ?? 'Student'} · {friend.status}</small></span></button>{#if friend.status === 'pending' && friend.incoming}<span class="request-actions"><button class="small-action" on:click={() => respond(friend, true)}>Accept</button><button class="text-link" on:click={() => respond(friend, false)}>Decline</button></span>{/if}</div>{/each}{/if}</div></section>
        <section class="student-panel message-panel">{#if selected}<div class="panel-title"><span>MESSAGES WITH {selected.displayName?.toUpperCase()}</span></div><div class="message-thread" aria-live="polite">{#each messages as message}<div class:mine={message.senderId === session.studentId} class="message-bubble"><p>{message.messageText}</p><small>{when(message.createdAt)}</small></div>{/each}{#if messages.length === 0}<p class="muted">Start a respectful dojo conversation.</p>{/if}</div><form class="message-compose" on:submit|preventDefault={send}><textarea bind:value={text} maxlength="2000" placeholder="Write a message…" aria-label="Message"></textarea><button class="primary-button" disabled={!text.trim()}>Send message</button></form>{:else}<div class="message-empty"><span class="rank-orb">✦</span><h2>Select a friend</h2><p>Direct messages stay behind authenticated API access and are available only to accepted friends.</p></div>{/if}</section>
      </aside>
    </div>
  </StudentShell>
{/if}
