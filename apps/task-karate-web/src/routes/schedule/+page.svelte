<script lang="ts">
  ...
  function openCheckIn(group: ScheduleGroup) { selectedGroup = group; selectedClass = group.primary; selectedClassId = String(group.primary.sessionId); selectedStudent = null; studentSearch = ''; attendanceMode = 'regular'; action = ''; }
  ...
  function closeCheckIn() { if (!checkInBusy) { selectedClass = null; selectedGroup = null; } }
  async function checkIn() { if (!selectedClass || !selectedStudent) return; const helper = attendanceMode === 'helper'; checkInBusy = true; action = ''; try { await api(`/api/student/public/schedule/${selectedClass.sessionId}/check-in`, { method: 'POST', body: JSON.stringify({ studentId: selectedStudent.studentId, confirmed: true, helper }) }); checkedIn = new Set([...checkedIn, selectedClass.sessionId]); action = `${selectedStudent.displayName} is checked in ${helper ? 'as a helper' : ''} for ${displayName(selectedClass)}.`; selectedClass = null; selectedGroup = null; } catch (e) { action = apiError(e); } finally { checkInBusy = false; } }
